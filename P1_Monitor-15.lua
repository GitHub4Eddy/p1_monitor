...
function todays_consumption:__init(dev)
  QuickAppChild.__init(self,dev)
  --self:trace("todays_consumption QuickappChild initiated, deviceId:",self.id)
...
function todays_production:__init(dev)
  QuickAppChild.__init(self,dev)
  --self:trace("todays_production QuickappChild initiated, deviceId:",self.id)
...
function waterflow:updateValue(data) 
  ...
  self:updateProperty("unit", "Liter")
  self:updateProperty("log", "")
end
...
function water:__init(dev)
  QuickAppChild.__init(self,dev)
  --self:trace("water QuickappChild initiated, deviceId:",self.id)
end
...
function QuickApp:updateLabels() -- Update the labels
  ...
  labelText = labelText .."Waterflow: " ..data.waterflow .." Liter" .."\n\n"
  ...
  labelText = labelText .."Gas: " ..data.gas .." m³ (Today: " ..data.today_gas .." m³)" .."\n"
...
function QuickApp:valuesWatermeter() -- Get the values from json file Watermeter
  ...
  data.waterflow = string.format("%.1f",jsonTable[1].WATERMETER_CONSUMPTION_LITER)
  ...
  self:logging(3,"data.waterflow_timestamp: " ..data.waterflow_timestamp .." time now: " ..os.time() .." difference: " ..os.time()-data.waterflow_timestamp)
  if os.time()-data.waterflow_timestamp > 60 then -- If measurement is older than 60 seconds then change measurement to zero
    data.waterflow = "0.0"
    self:logging(3,"Waterflow outdated, changed to zero")
  end
end
...
function QuickApp:valuesStatus() -- Get the values from json file Status
  ...
  data.consumption_L1 = string.format("%.1f",tonumber(jsonTable[74][2])*1000)
  data.consumption_L2 = string.format("%.1f",tonumber(jsonTable[75][2])*1000)
  data.consumption_L3 = string.format("%.1f",tonumber(jsonTable[76][2])*1000)
  data.production_L1 = string.format("%.1f",tonumber(jsonTable[77][2])*1000)
  data.production_L2 = string.format("%.1f",tonumber(jsonTable[78][2])*1000)
  data.production_L3 = string.format("%.1f",tonumber(jsonTable[79][2])*1000)
  data.L1_A = string.format("%.1f",jsonTable[100][2])
  data.L2_A = string.format("%.1f",jsonTable[101][2])
  data.L3_A = string.format("%.1f",jsonTable[102][2])
  data.L1_V = string.format("%.1f",jsonTable[103][2])
  data.L2_V = string.format("%.1f",jsonTable[104][2])
  data.L3_V = string.format("%.1f",jsonTable[105][2])  
  data.max_consumption = string.format("%.0f",tonumber(jsonTable[1][2])*1000)
  data.max_production = string.format("%.0f",tonumber(jsonTable[3][2])*1000)
  ...
  data.max_consumption_time = jsonTable[2][2]
  data.max_production_time = jsonTable[4][2]
  data.today_consumption_low = string.format("%.3f",jsonTable[8][2])
  data.today_consumption_high = string.format("%.3f",jsonTable[9][2])
  data.today_production_low = string.format("%.3f",jsonTable[10][2])
  data.today_production_high = string.format("%.3f",jsonTable[11][2])
  ...
  data.today_gas = string.format("%.3f",jsonTable[44][2])
...
function QuickApp:getData() -- Get data from P1 Monitor 
  ...
  fibaro.setTimeout(Interval, function() -- Checks every [Interval] seconds for new data (three loops: Smartmeter, Status and Watermeter)
    self:getData()
  end)
end 
...
function QuickApp:createVariables() -- Get all Quickapp Variables or create them
  ...
  pathStatus = "/api/v1/status" -- Default path Status
  pathWatermeter = "/api/v2/watermeter/minute?limit=1&json=object" -- Default path Watermeter
