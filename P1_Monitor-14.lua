...
-- This Quickapp retrieves power consumption, power production, gas and water usage from the (P1 Monitor) energy, gas and water meter 

-- Child Devices for Net Consumption, Consumption, Production, Gross Consumption, Device Consumption, Waterflow, Consumption High, Consumption Low, Production High, Production Low, Total Gas, Total Waterflow, Consumption L1 L2 L3, Ampere L1 L2 L3, Volt L1 L2 L3, Production L1 L2 and L3
...
function QuickApp:valuesWatermeter() -- Get the values from json file Watermeter
  ...
  data.waterflow = string.format("%.1f",jsonTable.WATERMETER_CONSUMPTION_LITER)
  data.water = string.format("%.3f",jsonTable.WATERMETER_CONSUMPTION_TOTAL_M3)
  data.waterflow_timestamp = tonumber(jsonTable.TIMESTAMP_UTC)
...
function QuickApp:valuesPhase() -- Get the values from json file Phase
  self:logging(3,"QuickApp:valuesPhase")
  ...
  data.L3_V = string.format("%.1f",jsonTable.L3_V)  
  data.production_L1 = string.format("%.1f",jsonTable.PRODUCTION_L1_W)
...
function QuickApp:getData() -- Get data from P1 Monitor 
  ...
  options = {
    ...
    success = function(response)
      ...
      if Mode == "Smartmeter" then
        ...
        Path = pathPhase -- Next path
        Mode = "Phase" -- Next mode
  ...
  fibaro.setTimeout(Interval, function() -- Checks every [Interval] seconds for new data (three loops: Smartmeter, Phase and Watermeter)
    self:getData()
  end)
end 
...
function QuickApp:setupChildDevices() -- Setup Child Devices
  ...
  if #cdevs == 0 then -- If no Child Devices, create them
    local initChildData = { 
      ...
      {className="waterflow", name="Water Flow", type="com.fibaro.multilevelSensor", value=0, unit="Liter"},
      ...
      {className="water", name="Water", type="com.fibaro.multilevelSensor", value=0, unit="m³"},
